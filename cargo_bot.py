...
(FROM_CITY, TO_CITY, GOODS_TYPE, PHOTO,
 DIMENSIONS, WEIGHT, PACKAGING, QUANTITY,
 PRICE, CONTACT, CONFIRM) = range(11)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Для оформления нажмите кнопку ниже 👇",
...
async def get_from_city(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Bu funksiya tugma bosilganda ishlaydi va birinchi savolni beradi
    await update.message.reply_text(
        "1️⃣ Yuk qayerdan olinadi? (Xitoyda shahar)\n"
        "Откуда забрать груз? (Город в Китае)\n\n"
        "Misol / Пример: Guangzhou, Yiwu, Shanghai",
...
async def get_to_city(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Misol / Пример: Toshkent, Samarqand, Andijon"
    )
    return GOODS_TYPE
...
async def get_goods_type(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "3️⃣ Qanday tovar?\n"
        "Что за товар?\n\n"
        "Misol / Пример: Kiyim, elektronika, mebel..."
    )
    return PHOTO
...
async def get_packaging(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "7️⃣ Qadoqlash turi\n"
        "Вид упаковки\n\n"
        "Misol / Пример: Quti, Qop, Plyonka, Yog'och qafas"
    )
    return QUANTITY
...
def main():
    ...
    conv = ConversationHandler(
        entry_points=[
            ...
            MessageHandler(
                filters.Regex("Yangi buyurtma|Новый заказ"),
                start
            )
        ],
        states={
            FROM_CITY:    [MessageHandler(filters.Regex("Buyurtma berish|Оформить"), get_from_city)],
            ...
            DIMENSIONS:   [MessageHandler(filters.PHOTO | (filters.TEXT & ~filters.COMMAND), get_dimensions)],
            ...
            CONFIRM + 1:  [MessageHandler(filters.TEXT & ~filters.COMMAND, send_to_admin)],
        },
